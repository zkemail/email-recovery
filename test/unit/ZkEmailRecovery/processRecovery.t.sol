...
contract ZkEmailRecovery_processRecovery_Test is UnitBase {
    ...
    function test_ProcessRecovery_RevertWhen_GuardianStatusIsREQUESTED()
        public
    {
        bytes[] memory subjectParams = new bytes[](3);
        subjectParams[0] = abi.encode(accountAddress);
        ...
        // Valid guardian but we haven't called acceptGuardian(), so the guardian status is still REQUESTED
        vm.expectRevert(
            abi.encodeWithSelector(
                IZkEmailRecovery.InvalidGuardianStatus.selector,
    ...
    function test_ProcessRecovery_InitiatesRecovery() public {
        ...
        assertEq(
            ...
            guardian1Weight + guardian2Weight
        );
    ...
    function test_ProcessRecovery_CompletesRecoveryIfDelayIsZero() public {
        ...
        // Since configureRecovery is already called in `onInstall`, we update the delay to be 0 here
