...
If the `UserOverrideableDKIMRegistry`, `Groth16Verifier`, `Verifier` and `EmailAuth` have not been deployed on your chosen network, you'll need to run two scripts. You'll need to clone [email-tx-builder/email-recovery](https://github.com/zkemail/email-tx-builder/tree/email-recovery), and then in this repo [email-recovery](https://github.com/zkemail/email-recovery). `email-tx-builder` includes the deployment scripts to deploy the DKIM regsitry, the Verifer and the generic ZK Email contracts that the recovery module uses. This repo `email-recovery` is holds the recovery-specific contracts.
