...
interface IRecoveryManager {
    ...
    struct PreviousRecoveryRequest {
        bytes32 previousGuardianInitiated; // the address of the guardian who initiated the previous
    ...
    struct RecoveryRequest {
        ...
        bytes32 recoveryDataHash; // the keccak256 hash of the recovery data used to execute the
            // recovery attempt
        EnumerableSet.Bytes32Set guardianVoted; // the set of guardians who have voted for the
    ...
    event GuardianAccepted(address indexed account, bytes indexed guardian);
    event RecoveryRequestStarted(
        address indexed account,
        bytes indexed guardian,
    ...
    // error GuardianVerifierNotSupported(address);
    error GuardianAlreadyVoted();
    error GuardianMustWaitForCooldown(bytes guardian);
    ...
    function hasGuardianVoted(address account, bytes32 guardian) external view returns (bool);
